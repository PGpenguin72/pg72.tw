---
title: TeaGod.exe Writeup (English ver.)
...
# Challenge TeaGod.exe(100)：

/worship

✨ Devout Worship ✨.

**PGpenguin72 (Tea God worshipper)** offers their sincere respects to the greatest and cutest Lady Tea God!

Cumulative worship count / Current streak: 666 times / 666 days

![TeaGod Avatar](./asset/TeaGod.png)

File：  
Originally uploaded challenge file: [TeaGod.exe](https://file.pg72.tw/share/v9036WOr)

The correct file that should have been uploaded: [TeaGod.exe](https://file.pg72.tw/share/TBuhE__L)

# Challenge Design Inspiration:
This challenge pays tribute to a Discord bot in our THJCC server, "Leader of the Greatest and Cutest Tea God Sister Church" (I forgot who made this bot). But most importantly, I just wanted to worship the Tea God (Lemon Tea).

I came up with this challenge because I wanted to make something like malware that would keep spawning pop-up windows. So I told ChatGPT about the idea, and it helped me build this Windows program.

But I uploaded the wrong file to CTFd—the one meant for internal testing—so this challenge became super easy (though it would have been very easy for AI to solve anyway==).

# Solution:
## Method 1
Download and open the file, and you will see this:
![Worship the Tea God](./asset/worship%20count.png)
![Worship the Tea God](./asset/worship%20tea%20god.png)
Then just click the button once, and you will solve it:
![Reward](./asset/reward.png)
...
Okay, Method 1 really was not my intended solution, so here is the approach I actually expected: Method 2.

Load it into Ghidra and reverse it:
...
```
### Here, we can see two very important sections:

#### 600-665
```cpp
// This is the text shown in the pop-up window
CreateWindowExW(0,L"STATIC",L"茶神降臨 // CLICK TO WORSHIP",0x50000001,0x14,0x12,0x186,
                0x1c,param_1,(HMENU)0x0,pHVar10,(LPVOID)0x0);
pHVar10 = GetModuleHandleW((LPCWSTR)0x0);
// The button will execute the routine associated with 0x1001
_DAT_1400080d0 =
    CreateWindowExW(0,L"BUTTON",L"WORSHIP TEA GOD",0x50000001,0x6e,0x177,0xd2,0x2a,param_1,
                    (HMENU)0x1001,pHVar10,(LPVOID)0x0);
```
...
```cpp
    ...
    // Check whether this is button 0x111
    if ((param_2 != 0x111) || (((uint)param_3 & 0xffff) != 0x1001)) goto LAB_140002078;
    
    // Increment the counter by one
    DAT_1400080c8 = DAT_1400080c8 + 1;
    
    // Refresh the display so the worship count shows the latest value.
    InvalidateRect(DAT_1400080c0,(RECT *)0x0,1);
    ...
    // Once it reaches the end, start decrypting
    if (DAT_1400080c8 != 0) {
      KillTimer(DAT_1400080c0,0xcafe);
      if (DAT_140008078 != DAT_140008080) {
        do {
...
```
`PTR_DAT_140005210` is a pointer table that maps to the following three locations:
| Pointer table address | Ciphertext address |
| --------    | ---------   |
| 0x140005210 | 0x1400051E6 |
| 0x140005218 | 0x1400051F2 | 
| 0x140005220 | 0x1400051FE |

Next, we open it in HxD:
![hxd](./asset/hxd.png)

Based on the table above, we can find three byte strings here:
...
Then, by checking `0x140005228`, we can find:
...
# Easter Egg:
When I was making this challenge, I actually hid a little detail in it.

If you look at the Flag for this challenge, you will notice that it can actually be turned into a link to a website:
::link{url="https://pastebin.com/R58uv133"}

This website contains the following text:
```txt
...
If you decode it from Base64, you get:
```txt
...
That mean:
```txt
Daily worship to the greatest, prettiest, and cutest Tea God Sister, who can not only write the cross-platform TeaGod OS with her feet, but also handle decompilation in her head with TeaGod Decompiler, tune a five-trillion-parameter TeaGod Agent by hand, build TeaGod Protocol with her knees, finish every CTF challenge with a single look, and even help create challenges—the tech god, Chairman of Lin Hai International, and the Lu Yu of cybersecurity 🛐 🛐 🛐
```
These are some of the phrases we use to worship the Tea God. We used to post them in the THJCC server, so if you are interested in worshipping the Tea God with us, come post them too :D
